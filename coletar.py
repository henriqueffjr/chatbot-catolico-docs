...
import redis
from v22 import Config, CrawlerStats, baixar_e_processar
import logging
import gc
import psutil
import time
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.info("Índice Elasticsearch criado com sucesso")

        # URLs iniciais para crawling
        urls_iniciais = [
            "https://www.vatican.va/content/vatican/pt.html",
            "https://www.vatican.va/content/francesco/pt.html"
        ]

        # Define o tamanho máximo de memória (em bytes) - ajuste conforme necessário
        max_memory = 1024 * 1024 * 1024  # 1GB

        for url in urls_iniciais:
            if psutil.Process().memory_info().rss > max_memory:
                logger.warning("Limite de memória atingido, pausando por 30 segundos para liberar recursos.")
                time.sleep(30)
                gc.collect()
            ...
            try:
                ...
                if metadata and metadata.links:
                    ...
                    # Processa links em lotes pequenos para evitar alto consumo de memória
                    batch_size = 5
                    for i in range(0, len(metadata.links), batch_size):
                        batch = metadata.links[i:i + batch_size]

                        if psutil.Process().memory_info().rss > max_memory:
                            logger.warning("Limite de memória atingido no lote, pausando por 30 segundos.")
                            time.sleep(30)
                            gc.collect()
        ...
        # Exibe relatório
        logger.info("\n" + "="*70)
        logger.info("RELATÓRIO DETALHADO DO CRAWLER")
        logger.info("="*70)
        logger.info(f"Tempo de execução: {horas}h {minutos}m")
        logger.info(f"URLs processadas: {stats.urls_processadas}")
        if stats.urls_processadas > 0:
            taxa_sucesso = ((stats.urls_processadas - stats.urls_falhas) / stats.urls_processadas) * 100
            logger.info(f"Taxa de sucesso: {taxa_sucesso:.1f}%")
            velocidade = stats.bytes_baixados / (tempo_total.seconds or 1) / 1024
            logger.info(f"Velocidade média: {velocidade:.2f} KB/s")
            media_links = (stats.links_encontrados / stats.urls_processadas if stats.urls_processadas > 0 else 0)
            logger.info(f"Média de links por página: {media_links:.1f}")

        logger.info(f"Documentos importantes: {stats.docs_importantes}")
        logger.info(f"\nErros:")
        logger.info(f"  404: {stats.erros_404}")
        logger.info(f"  Timeout: {stats.erros_timeout}")
        logger.info(f"  Rate Limit: {stats.erros_rate_limit}")

        # Lista as categorias mais comuns
        if stats.categorias:
            logger.info(f"\nTop Categorias:")
            for cat, count in sorted(stats.categorias.items(), key=lambda x: x[1], reverse=True):
